...
import config
from config import RANK_BIAS, SELECTIONPROB, SELECTIONOP, RANK_BASE
...
class MapElites(ABC):
    ...
    def __init__(self, case_study_file, budget, logs=True):
        """
        ...
        """
        self.log_flag = logs
        self.log_dir_name = Folder.DST
        self.mission = case_study_file
        self.elapsed_time = 0
    ...
    def run(self):
        ...
        if self.log_flag == True:
            ...
            Path(self.log_dir_name).mkdir(parents=True, exist_ok=True)

            log_to = f"{self.log_dir_name}/logs.txt"
            debug = f"{self.log_dir_name}/debug.txt"
        ...
        while config.EXEC_DONE < self.budget:            
            ...
            self.solutions[idx].selected_counter += 1  
            parent = self.solutions[idx]
            log.info(f"Iteration {iter}: Selecting individual {parent.id} at {parent.features} with rank: {parent.rank} and seed: {parent.seed}")  
        ...
        misbehaviours = self.extract_misbehaviours()
        return misbehaviours


    def extract_misbehaviours(self):
        test_cases = []
        for x in enumerate(self.performances.items()): 
            ...
            if x[1][1] < 1.5:
                test_cases.append(self.solutions[x[1][0]].test)
        return test_cases
    ...
    def extract_results(self, iterations, execution_time):
        log_dir_path = Path(f"{self.log_dir_name}")
        ...
        dst = f"{self.log_dir_name}/report.json"
        ...
        for feature1, feature2 in itertools.combinations(self.feature_dimensions, 2):         
            ...
            dst = f"{self.log_dir_name}/report_" + feature1.name + "_" + feature2.name + '.json'
            with open(dst, 'w') as f:
                (json.dump(report, f, sort_keys=False, indent=4))

            self.plot_map_of_elites(_performances, self.log_dir_name, feature1, feature2)       
    ...
    def place_in_mapelites(self, x, parent, iter):
        ...
        if b in self.performances:
            if self.place_operator(perf, self.performances[b]):
                log.info(f"Iteration {iter}: Replacing individual {x.id} at {b} with perf: {perf} and seed: {x.seed}")
                
                if parent is not None:
                    self.solutions[parent].placed_mutant += 1
                self.performances[b] = perf
                self.solutions[b] = x
                status = "Replace"
            else:
                log.info(f"Iteration {iter}: Rejecting individual {x.id} at {b} with perf: {perf} in favor of {self.performances[b]}")
                status = "Reject"
        else:

            log.info(f"Iteration {iter}: Placing individual {x.id} at {b} with perf: {perf} and seed: {x.seed}")                    
            status = "Place"
            if parent is not None:
                self.solutions[parent].placed_mutant += 1
            self.performances[b] = perf
    ...
    def rank_in_mapelites(self, b, parent, status):
        if status == "Place" or status == "Replace":
            if RANK_BASE == "perf":
                self.solutions[b].rank = self.performances[b]
            elif RANK_BASE == "density":
                self.solutions[b].rank = compute_sparseness(self.solutions, self.solutions[b])
                neighbors = get_neighbors(b)
                # compute rank for all neighbors
                for neighbor in neighbors:
                    if neighbor in self.solutions:      
                        self.solutions[neighbor].rank = compute_sparseness(self.solutions, self.solutions[neighbor])
            else:
                if parent is not None and self.solutions[parent].selected_counter > 0:
                    self.solutions[parent].rank = self.solutions[parent].placed_mutant / self.solutions[parent].selected_counter
                self.solutions[b].rank = 1.0
    ...
    def rank_selection(self, individuals=1):
        ...
        def _get_sparse_index():
            ...
            if RANK_BASE == "perf":
                ...
                solutions = [value for key, value in self.solutions.items() if value.rank >= 0]
                ...
                solutions = [value for key, value in self.solutions.items()]
                solutions.sort(key=lambda x: x.rank, reverse=True)
